...
def create_rotate(path_file):
    ...
    message = {
        'file_name': output_folder + name + NEW + ext,
        'operation': 'rotacionada'
    }
...
def send_notification(message):
    producer = KafkaProducer(
        bootstrap_servers='localhost:9092',  # Configuração do Kafka
        value_serializer=lambda v: json.dumps(v).encode('utf-8') # padroniza a mensagem
    )
